...
function mapComment(c, profiles, replyToUserId) {
...
function mapToMomentItem(moment, profiles, replyMap) {
    const likes = (moment.likes ?? []).map((l) => mapLike(l, profiles));
...
function collectUserIds(moments) {
    const ids = new Set();
...
async function fetchProfiles(prisma, redis, uids) {
    const map = new Map();
    ...
    if (missed.length > 0) {
        ...
        for (const u of users) {
            const profile = { id: u.id, nickname: u.nickname, avatar: u.avatar };
            map.set(u.id, profile);
            // Write back to Redis
            if (redis) {
                try {
                    await redis.set(RedisKeys.userProfile(u.id), JSON.stringify(profile), 'EX', RedisKeys.USER_PROFILE_TTL);
                }
                catch { /* ignore */ }
            }
        }
...
async function fetchReplyUserMap(prisma, comments) {
    const replyIds = [...new Set(comments.map((c) => c.replyToId).filter(Boolean))];
    ...
    for (const r of replied)
        map.set(r.id, r.userId);
    return map;
}
/** Push a moment ID to all friends' timeline caches */
async function pushToFriendsTimelines(redis, prisma, authorId, momentId, createdAt) {
    if (!redis)
        return;
...
async function removeFromTimelines(redis, prisma, authorId, momentId) {
    if (!redis)
        return;
    const reverseContacts = await prisma.contact.findMany({
        where: { contactId: authorId, status: 'active' },
...
async function getFriendIds(prisma, userId) {
...
async function areFriends(prisma, userId, otherId) {
...
// ─── Service Factory ────────────────────────────────────────────────────────
export function createMomentsService(deps) {
    ...
    // ─── Delete Moment ──────────────────────────────────────────────────────
    async function deleteMoment(input) {
    ...
    // ─── Get Moment By ID ───────────────────────────────────────────────────
    async function getMomentById(momentId, userId) {
    ...
    // ─── Get User Moments ───────────────────────────────────────────────────
    async function getUserMoments(targetUid, query) {
        ...
        const allComments = moments.flatMap((m) => m.comments ?? []);
    ...
    // ─── Toggle Like ────────────────────────────────────────────────────────
    async function toggleLikeMoment(input) {
        const moment = await prisma.moment.findUnique({ where: { id: input.momentId } });
        if (!moment) {
            throw new MomentError(ErrorCode.MOMENT_NOT_FOUND, '朋友圈不存在');
        }
    ...
    async function likeMoment(input) {
        const moment = await prisma.moment.findUnique({ where: { id: input.momentId } });
        if (!moment) {
            throw new MomentError(ErrorCode.MOMENT_NOT_FOUND, '朋友圈不存在');
        }
    ...
    async function unlikeMoment(input) {
        const existing = await prisma.momentLike.findUnique({
            where: { momentId_userId: { momentId: input.momentId, userId: input.userId } },
        });
        if (!existing) {
