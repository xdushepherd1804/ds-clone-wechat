...
import { ErrorCode, ErrorMessage, hashPassword, verifyPassword, isValidUsername, isValidPassword, isValidNickname, sanitizeText, } from '@wechat-clone/shared';
...
export function createAuthService(deps) {
    ...
    const store = deps.sessionStore ?? (redis ? createRedisSessionStore(redis) : new InMemorySessionStore());
    ...
    // ─── Update Profile ────────────────────────────────────────────────────
    async function updateProfile(userId, req) {
        const data = {};
    ...
    // ─── Change Password ───────────────────────────────────────────────────
    async function changePassword(userId, req) {
        const user = await prisma.user.findUnique({ where: { id: userId } });
        if (!user) {
            throw new AuthError(ErrorCode.NOT_FOUND, '用户不存在');
        }
    ...
    // ─── Rate Limiter ──────────────────────────────────────────────────────
    async function checkRateLimit(action, identifier, windowSeconds, maxAttempts) {
        if (!redis)
            return; // Skip rate limiting without Redis
