...
// ─── Helpers ───────────────────────────────────────────────────────────────
async function requireOwner(prisma, groupId, userId) {
...
async function requireAdminOrOwner(prisma, groupId, userId) {
    const member = await prisma.groupMember.findUnique({
        where: { groupId_userId: { groupId, userId } },
    });
...
function mapToGroupMember(member) {
    return {
        id: member.id,
...
export function createGroupService(deps) {
    ...
    // ─── Update Group ───────────────────────────────────────────────────────
    async function updateGroup(groupId, operatorId, input) {
    ...
    async function addMembers(input) {
        ...
        await prisma.$transaction([
            ...newUserIds.map((userId) => prisma.groupMember.create({
    ...
    async function updateMemberNickname(input) {
    ...
    // ─── Approve / Reject Join ─────────────────────────────────────────────
    async function approveJoin(groupId, userId, operatorId, action) {
        await requireAdminOrOwner(prisma, groupId, operatorId);
    ...
    // ─── Set Announcement ──────────────────────────────────────────────────
    async function setAnnouncement(groupId, operatorId, announcement) {
        await requireAdminOrOwner(prisma, groupId, operatorId);
        const group = await prisma.group.update({
            where: { id: groupId },
            data: { announcement },
    ...
    // ─── Validate Mentions ─────────────────────────────────────────────────
    async function validateMentions(input) {
